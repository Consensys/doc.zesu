---
title: Obtain the guest program
sidebar_label: Obtain the guest program
description: Where to get the Zesu zkVM guest program artifact and how to integrate it into a zkVM host.
keywords:
  [
    zesu,
    zkVM,
    guest program,
    ELF,
    rv64im,
    zesu.rv64im.o,
    GitHub Releases,
    integrate,
  ]
---

import GlossaryTerm from '@theme/GlossaryTerm'

Zesu is distributed as a pre-built relocatable <GlossaryTerm term="ELF object">Executable and Linkable Format (ELF)
object</GlossaryTerm>, `zesu.rv64im.o`.
To use it, your zkVM platform needs to provide implementations of the platform-specific
functions Zesu depends on, then link them together to produce a runnable guest binary.

## Prerequisites

- A zkVM that targets the <GlossaryTerm term="rv64im">`rv64im`</GlossaryTerm> (RISC-V 64-bit base integer with multiply extension)
  instruction set.

## Download the artifact

[Download](https://github.com/Consensys/zesu/releases) the pre-built `zesu.rv64im.o`
artifact for the release you want to integrate.

## Integrate the artifact

Your zkVM host integration implements the platform-specific functions and links them
against `zesu.rv64im.o` to produce the final guest binary your zkVM runs.
Linking conventions are platform-specific. Your zkVM target determines the linker script,
entry point, heap layout, and accelerator wiring.

:::note
The [`zesu-zkvm` repository](https://github.com/Consensys/zesu-zkvm) contains example
integrations for the Zisk, OpenVM, and Linea zkVMs.
It is a stop-gap for zkVMs that don't yet implement the common guest interface, and shows
how to wire a non-compliant zkVM to Zesu.
As zkVMs adopt the [zkVM standards](https://github.com/eth-act/zkvm-standards), they can
satisfy the symbol contract directly and the example integrations become unnecessary.
:::

The [zkVM symbol reference](../reference/zkvm-symbols.mdx) contains complete tables of the I/O,
runtime, and accelerator symbols the host object must implement.

## Next steps

- [Architecture](../concepts/architecture.mdx): the full proving pipeline and why the ELF
  design was chosen.
- [zkVM symbol reference](../reference/zkvm-symbols.mdx): complete tables of the I/O,
  runtime, and accelerator symbols the host object must implement.

