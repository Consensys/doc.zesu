...
---
flowchart LR
    subgraph input["Private input (StatelessInput)\nnot visible to verifiers"]
    ...
    subgraph pubout["Public output (41 bytes)\nvisible to verifiers"]
        O1["new_payload_request_root (32B)"]
        O2["chain_id (8B)"]
        O3["successful_validation (1B)"]
    end
    ...
    zkvm -->|proof| proof
    classDef blueBox fill:#dbeafe,stroke:#2563eb,color:#1e40af
    classDef grayBox fill:#e8e8e8,stroke:#969696,color:#303030
...
Zesu serializes this as a 41-byte SSZ commitment:
